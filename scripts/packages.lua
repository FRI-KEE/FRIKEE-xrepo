-- This file is modified from the official xmake-repo
...
function main(opt)
    ...
    for _, packagedir in ipairs(os.dirs(path.join("packages", "*", "*"))) do
        ...
        if instance then
            -- We don't need to support as much platforms as the official repo
            for _, plat in ipairs({"windows", "linux", "wasm"}) do
